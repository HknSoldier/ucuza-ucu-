...
class AnalysisEngine:
    def scan_route(self, origin, dest, hard_limit=None):
        ...
        # 1 Yıllık Tarama (Rastgele 4 dönem seç)
        attempt_dates = []
        for _ in range(4):
            days_future = random.randint(14, 300)
            attempt_dates.append(today + timedelta(days=days_future))

        for date_obj in attempt_dates:
            stay_days = random.randint(3, 14) # 3-14 Günlük Tatil
            ...
            try:
                ...
                if result and result.flights:
                    ...
                    if hard_limit and price > hard_limit: continue
